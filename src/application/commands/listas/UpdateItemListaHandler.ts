...
export class UpdateItemListaHandler {
  ...
  async execute(
    command: UpdateItemListaCommand,
    ...
    if (command.nome !== undefined) item.updateNome(command.nome);
    if (command.quantidade !== undefined)
      item.updateQuantidade(command.quantidade);
    if (command.valorUnitario !== undefined)
      item.updateValorUnitario(command.valorUnitario);
    if (command.categoriaId !== undefined)
      item.updateCategoria(command.categoriaId);
